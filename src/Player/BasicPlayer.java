...
public class BasicPlayer implements BasicController, Runnable
{
	...
	public static int modelRow=0;
	public static int viewRow=0;
	//ControlPanel cp;
	String album;
	...
	{
		ControlPanel.pp.setIcon(ControlPanel.play);
		ControlPanel.pp.setPressedIcon(ControlPanel.playpressed);
		ControlPanel.pp.setRolloverIcon(ControlPanel.playrollover);
	...
	{
		ControlPanel.pp.setIcon(ControlPanel.pause);
		ControlPanel.pp.setPressedIcon(ControlPanel.pausepressed);
		ControlPanel.pp.setRolloverIcon(ControlPanel.pauserollover);
		if (m_line != null)
		{
			if (m_status == PAUSED)
			{
	...
	public int getPlayingRow()	{
		int viewRow = -1;
		//	if (uSic.nowPlaying.getText().isEmpty())	{
		//		return viewRow;
		//	} else	{
		for (int i = 0; i < FileTraverse.list.size(); i++)	{
			try	{
				if (LibraryPanel.model.getValueAt(i, 1).toString().equals(title) && 
						LibraryPanel.model.getValueAt(i, 4).toString().equals(album))	{
					viewRow = LibraryPanel.table.convertRowIndexToView(i);
				}
	 ...
	 * Starts playback.
	 */
	public void startPlayback() throws BasicPlayerException
	{	
		//modelRow = LibraryPanel.table.convertRowIndexToModel(LibraryPanel.table.getSelectedRow());
		String[] lib = LibraryPanel.listLib();
		//File file = new File(lib[modelRow]);
		LibraryPanel.table.getSelectionModel().setSelectionInterval(modelRow, modelRow);
		//	currentFile = new File(lib[modelRow]);
		//updateNowPlaying();
		//	viewRow = LibraryPanel.table.convertRowIndexToView(LibraryPanel.table.getSelectedRow());
		//JOptionPane.showMessageDialog(null, LibraryPanel.table.convertRowIndexToView(LibraryPanel.table.getSelectedRow()));
		...
		viewRow = getPlayingRow();
		LibraryPanel.table.getSelectionModel().setSelectionInterval(viewRow,viewRow);
		scrollToVisible(LibraryPanel.table, viewRow, 0);

		File file = new File(lib[LibraryPanel.table.convertRowIndexToModel(LibraryPanel.table.getSelectedRow())]);
		currentFile = file;
		control.open(currentFile);
		//setGain(ControlPanel.volume.getValue()*0.01);
		ControlPanel.pp.setIcon(ControlPanel.pause);
		ControlPanel.pp.setPressedIcon(ControlPanel.pausepressed);
		ControlPanel.pp.setRolloverIcon(ControlPanel.pauserollover);
	...
	{
		BasicPlayerEventLauncher trigger = new BasicPlayerEventLauncher(code, position, value, description, new ArrayList<BasicPlayerListener>(m_listeners), this);
		trigger.start();
	}
	...
	/**
	 * @see jlgui.basicplayer.BasicController#seek(long)
	 */
	...
	/**
	 * @see jlgui.basicplayer.BasicController#play()
	 */
	public void play()
	{
		modelRow = LibraryPanel.table.convertRowIndexToModel(LibraryPanel.table.getSelectedRow());
		...
		ControlPanel.seek.setMaximum(getTrackLengthSec());
		int delay = 1000;
		int interval = 1000;
		try {
			startPlayback();
		...
		timer.scheduleAtFixedRate(new TimerTask() {
			public void run() {
				if (isPlaying())	{
					ControlPanel.seek.setValue(ControlPanel.seek.getValue()+1);
					ControlPanel.seek.validate();
				} else if (isStopped()){
	...
	/**
	 * @see jlgui.basicplayer.BasicController#stop()
	 */
	...
	/**
	 * @see jlgui.basicplayer.BasicController#pause()
	 */
	...
	/**
	 * @see jlgui.basicplayer.BasicController#resume()
	 */
	...
	public void forward()	{
		if ((getPlayingRow()==LibraryPanel.table.getRowCount()-1))	{	//if last song
			if (isLoopAll())	{		//if loop all...
				previousRow = modelRow;
				modelRow = LibraryPanel.table.convertRowIndexToModel(0);//set current to first song
			}
		} else	{
			if (isLoopOnce())	{		//if loop once...
				previousRow = modelRow;
				modelRow = LibraryPanel.table.convertRowIndexToModel(getPlayingRow());
			} else	{					//if not loop once...
				if (isShuffle())	{	//if shuffle...
					previousRow = currentRow;
					Random random = new Random();
					int pick = random.nextInt(LibraryPanel.table.getRowCount());
					modelRow = LibraryPanel.table.convertRowIndexToModel(pick);
				} else	{ //if not loop once and not shuffle
					previousRow = modelRow;
					//System.out.println("neither");
					modelRow = LibraryPanel.table.convertRowIndexToModel(getPlayingRow()+1);//HERE
				}
			}
		}
		try {
	...
	public void back()	{
		if ((getPlayingRow()) == 0)	{	
			if (isLoopAll())	{
				modelRow = LibraryPanel.table.convertRowIndexToModel(LibraryPanel.table.getRowCount()-1);
			}
		} else	{
			if (isLoopOnce())	{
				previousRow = modelRow;
				...
				if (isShuffle())	{
					...
					modelRow = LibraryPanel.table.convertRowIndexToModel(pick);
					previousRow = currentRow;
				} else	{	//if not looping or shuffling
					modelRow = LibraryPanel.table.convertRowIndexToModel(getPlayingRow()-1);
				}
			}
		}
		try {
	...
	public float getTotalFrames() 	{
		...
		try {
			...
			File file = new File(lib[LibraryPanel.table.convertRowIndexToModel(LibraryPanel.table.getSelectedRow())]);
			currentFile = file;
			format = AudioSystem.getAudioFileFormat(currentFile);
			AudioFormat af = format.getFormat();
			totalFrames = af.getFrameRate() * getTrackLengthSec();
		} catch (Exception e) {
			e.printStackTrace();
		} 
		return totalFrames;
	}
	public int getTrackLengthSec() 	{
		AudioFile f = null;
		AudioHeader d = null;
		try {
			f = AudioFileIO.read(currentFile);
	...
	public float getFrameSize() 	{
		float frameSize = 0;
		try {
			frameSize = getTotalFrames()/getTrackLengthSec();
		} catch (Exception e) {
			e.printStackTrace();
		} 
		return frameSize;
	...
	public boolean isShuffle()	{
		if (ControlPanel.shuffle.getIcon()==ControlPanel.shuffleicon)	{
			return true;
		} else	{
			return false;
		}
	...
	public boolean isLoopAll()	{
		if (ControlPanel.loop.getIcon()==ControlPanel.loopAll)	{
			return true;
		}	else	{
			return false;
		}
	...
	public boolean isLoopOnce()	{
		if (ControlPanel.loop.getIcon()==ControlPanel.loopOnce)	{
			return true;
		}	else	{
			return false;
		}
